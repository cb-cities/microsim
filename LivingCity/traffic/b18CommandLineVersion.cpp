...
void B18CommandLineVersion::runB18Simulation() {
  ...
  RoadGraphB2018 graph_loader = RoadGraphB2018(networkPathSP, odDemandPath);
  ...
  //
  //      std::vector<long> edge_nodes = {std::get<0>(x.first),
  //                                      std::get<1>(x.first)};
  //      edge_vals.emplace_back(edge_nodes);
  //      // std::cout << "origin = " << sources[x] << " \n";
  //      // std::cout << "Start node id = " << edge_nodes[0] << " End node id =
  //      "
  //      // << edge_nodes[1] << "\n";
  //    }
  ...
  //    MTC::accessibility::Accessibility *graph_ch =
  //        new MTC::accessibility::Accessibility(
  //            (int)street_graph->vertices_data_.size(), edge_vals,
  //            edge_weights, false);
  //
  ...
  //               (all_paths[i + 1] == -1)) { // if current is -1 and next is
  //               -1,
  ...
  //    }
  //  }
  //  // std::cout << "person_to_init_edge size " <<
  //  // street_graph->person_to_init_edge_.size() << "\n"; std::cout << "# of
  //  // paths = " << all_paths.size() << "\n"; std::cout << "Shortest path time
  //  =
  //  // " << duration.count() << " ms \n";
  //
