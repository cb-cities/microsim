...
void B18TrafficOD::randomPerson(int p, B18TrafficPerson &person,
  ...
  for (boost::tie(vi, viEnd) = boost::vertices(roadGraph); vi != viEnd; ++vi) {
...
void B18TrafficOD::createRandomPeople(
    int numberPerGen, float startTimeH, float endTimeH,
    std::vector<B18TrafficPerson> &trafficPersonVec,
...
float sampleFileDistribution() {
  ...
  if (fileDistributionInitialized == false) {
    ...
    if (!inputFile.open(QIODevice::ReadOnly)) {
      printf("for fileDistributionInitialized must exist file "
             "berkeley_2018/HtoW_trips.csv\n");
      exit(-1);
    }
    ...
    while (!in.atEnd()) {
      ...
      if (fields.size() >= 3) {
        ...
        // hToWDistribution[targetBucket], targetBucket, numBuckets);
      }
    }
 ...
 void B18TrafficOD::loadB18TrafficPeople(
    ...
    RoadGraph::roadBGLGraph_BI &roadGraph, const int limitNumPeople, const
...
//  boost::math::non_central_t_distribution<> > var(rng, td);
...
//      (trafficPersonVec[p].time_departure / 3600.0f)); numPeople++;
//    }
//  }
...
//      3600.0f)); float t = (trafficPersonVec[p].time_departure / 3600.0f) -
//      startTimeH;
//
...
//    for (int p = 0; p < trafficPersonVec.size(); p++) {
//      // printf("depart %.2f\n", (trafficPersonVec[p].time_departure /
//      3600.0f)); float time = (trafficPersonVec[p].time_departure / 3600.0f);
//      int targetBucket = (time - startSamples) * numBucketsPerHour;
//
//      if (targetBucket < 0 || targetBucket >= hToWDistribution.size()) {
//        continue;
...
void B18TrafficOD::loadB18TrafficPeopleSP(
  ...
  for (int d = 0;
       (d < totalNumPeople) && (numPeople < totalNumPeople);
       d++) {
    int odNumPeople = std::min<int>(totalNumPeople - numPeople,
                                    demand[d].num_people);
    // printf("odNumPeople = %d\n", odNumPeople);
